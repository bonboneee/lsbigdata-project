...
#0.05(유의수준) 보다 크므로, 귀무가설을 기각하지 못한다.
...
##분산 다른 경우: 웰치스 t 검정 #equal_var = False 해주면 됨 
##귀무가설 vs 대립가설
...
#ttest_ind(m_tab2["score"], f_tab2["score"], alternative = "greater", equal_var = True)과 동일 
...
df_wide = df_long.pivot_table(columns = "group", values="score") #aggfuncion이라는 mean으로 애가 숨어있음
df_long.pivot_table(columns = "group", values="score", aggfunc = "mean") #위의 식이랑 결과 같음 
